...
class WC_Pre_Orders_Manager {
	...
	public function check_for_pre_orders_to_complete() {
		...
		foreach ( $query->posts as $order_post ) {
			...
			$availability_timestamp = (int) get_post_meta( $product->is_type( 'variation' ) && version_compare( WC_VERSION, '3.0', '>=' ) ? $product->get_parent_id() : $product->get_id(), '_wc_pre_orders_availability_datetime', true );
			...
			if ( $availability_timestamp <= $time_now ) {
